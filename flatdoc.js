...
(function($) {
   ...
   *     Flatdoc.file([ 'http://path/to/url', ... ])
   *     Flatdoc.path('/path/to/url')
   *     Flatdoc.path('/path/to/url', 'README.md')
   */
  ...
  /**
   * Path fetcher function.
   * Fetches from a give url path.  Defaults to README.md unless specified.
   * The links in the path will be follow-able.
   */
  Flatdoc.path = function(opts) {
    ...
    var loadPath = function() {
      var hashloc = (location.hash || '').replace(/^#/,'');
      if (!hashloc.includes('.')) { // to distiguish from an anchor or file
        hashloc = '';
      }
      opts.file = (hashloc || "README.md");
      return Flatdoc.run({path: opts.path, fetcher: Flatdoc.file(opts.path + opts.file)});
    };
   ...
   */
  Parser.parse = function(source, highlight, renderer) {
    marked = exports.marked;

    Parser.setMarkedOptions(highlight);

    var html = $("<div>" + marked(source, { renderer: renderer }));
  ...
  Runner.prototype.run = function() {
    ...
    doc.fetcher(function(err, markdown) {
      ...
      var data = Flatdoc.parser.parse(markdown, doc.highlight, doc.Renderer());
  ...
  Runner.prototype.Renderer = function() {
    var doc = this;
    ...
    renderer.link = function(href, title, text) {
      ...
      if (href[0] != '/' && href.indexOf('//') == -1 && doc.path.length > 0) {
        href = '#' + href;
      }
      var out = '<a href="' + href + '"';
      if (title) {
    ...
    renderer.image = function(href, title, text) {
      console.log("href: " + href + " title: " + title + " text:" + text);
      href = doc.path + href;
      var out = '<a href="' + href + '" target="_blank"><img class="thumbnail" src="' + href + '" alt="' + text + '"';
      if (title) {
        out += ' title="' + title + '"';
      }
